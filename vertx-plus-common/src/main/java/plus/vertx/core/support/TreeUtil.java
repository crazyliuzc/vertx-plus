...
import plus.vertx.core.support.tree.*;
...
public class TreeUtil {
	...
	public static <T, E> List<Tree<E>> build(List<T> list, E parentId, TreeNodeConfig treeNodeConfig, NodeParser<T, E> nodeParser) {
		final List<Tree<E>> treeList = Lists.newArrayList();
		Tree<E> tree;
		for (T obj : list) {
		...
		List<Tree<E>> finalTreeList = Lists.newArrayList();
	...
	private static <T> void innerBuild(List<Tree<T>> treeNodes, Tree<T> parentNode, int deep, Integer maxDeep) {
		...
		if (ValidateUtil.isEmpty(treeNodes)) {
			return;
		}
		//maxDeep 可能为空
	...
	public static <T> Tree<T> getNode(Tree<T> node, T id) {
		if (Objects.equal(id, node.getId())) {
			return node;
		}
	 ...
	 *
	 * <p>
	 * 比如有个人在研发一部，他上面有研发中心，接着上面有技术中心<br>
