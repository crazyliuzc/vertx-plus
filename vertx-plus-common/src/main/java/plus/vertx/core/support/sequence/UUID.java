 ...
 *
 * <p>
 * variant 字段包含一个表示 UUID 布局的值。以上描述的位布局仅在 UUID 的 variant 值为 2（表示 Leach-Salz
 * 变体）时才有效。
 *
 * <p>
 * version 字段保存描述此 UUID 类型的值。有 4 种不同的基本 UUID 类型：基于时间的 UUID、DCE 安全 UUID、基于名称的
 * UUID 和随机生成的 UUID。<br>
...
public final class UUID implements java.io.Serializable, Comparable<UUID> {
    ...
    public static UUID randomUUID(boolean isSecure) {
        final Random ng = isSecure ? Holder.numberGenerator : ThreadLocalRandom.current();
     ...
     * <li>0 为 NCS 向后兼容保留
     * <li>2
     * <a href="http://www.ietf.org/rfc/rfc4122.txt">IETF&nbsp;RFC&nbsp;4122</a>(Leach-Salz),
     * 用于此类
     * <li>6 保留，微软向后兼容
     * <li>7 保留供以后定义使用
     * </ul>
     ...
     *
     * @return 时间戳值
     * @throws UnsupportedOperationException 如果此 {@code UUID} 不是 version 为 1 的
     * UUID。
     */
